...
function EditSupplier() {
    ...
    let {id} = useParams()
    const getSupplier = () => {
        axios.get('/suppliers/findById/' + id)
            .then(resp => {
                setSupplierEdit(resp.data)
            })
            .catch(() => {
                navigate("/")
            })
    ...
    const [supplierEdit, setSupplierEdit] = useState({
        ...
        company: '',
        taxCode: '',
        groupSupplierId: groupSupplierId.groupSupplierId
    })
    const onSave = (e) => {
        let data = {
            supplierId: supplierEdit.supplierId,
            supplierName: supplierEdit.supplierName,
            phoneNumber: supplierEdit.phoneNumber,
            address: supplierEdit.address,
            email: supplierEdit.email,
            company: supplierEdit.company,
            taxCode: supplierEdit.taxCode,
            groupSupplierId: supplierEdit.groupSupplierId,
        }
        const headers = {
            'Authorization': 'Bearer ' + localStorage.getItem('token'),
        }

        axios.post('/suppliers', data, {headers: headers})
            .then(() => {
                navigate('/supplier')
            })
    }
    ...
    const inputData = (e) => {
        ...
        setGroupSupplierId({...groupSupplierId, [e.target.name]: e.target.value})
    }
    //------------------------------------------------------------------------------------------------------------------
    ...
    return (<div className='customDiv' style={{height: '950px'}}>
            ...
            <MDBContainer>
                <MDBRow>
                    <MDBCol md={2}></MDBCol>
                    <MDBCol md={9} className='mt-5 bg-white rounded-5'>
                        <MDBContainer>
                            <MDBRow>
                                <MDBCol className='mt-3 ms-3 mb-4'>
                                    <h4><b>Sửa nhà cung cấp</b></h4>
                                </MDBCol>
                            </MDBRow>
                            <MDBRow>
                                <MDBCol md="6">
                            ...
                            <MDBRow>
                                <MDBCol className='mt-5 mb-5 d-flex justify-content-end'>
                                    ...
                                    <Link to={'/Supplier'} className='btn btn-dark ms-2'>
                                        <b>Bỏ qua</b>
                                    </Link>
                                </MDBCol>
                            </MDBRow>
