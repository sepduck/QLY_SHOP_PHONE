...
//  Delete customer -----------------------------------------------------------------------------------------------------
    ...
    const deleteCustomer = (e) => {
        ...
        axios.delete('/customer/' + e.target.value, {headers: headers})
            .then(() => {
...
//  List customer ------------------------------------------------------------------------------------------------------
    ...
    const getListCustomer = () => {
        ...
        axios.get('/customer', {headers: headers})
            .then(resp => {
                setListCustomer(resp.data.data)
            })
            .catch(error => {
                console.log(error)
                navigate("/")
...
//  List gender ------------------------------------------------------------------------------------------------
    ...
    const getListGender = () => {
        ...
        axios.get('/gender', {headers: headers})
            .then(resp => {
                setListGender(resp.data.data)
            })
            .catch(error => {
                console.log(error)
                navigate("/")
    ...
    const inputDataSearchCustomerGender = (e) => {
        ...
        setSearchGenderInitial({...searchGenderInitial, [e.target.name]: e.target.value})
    }
    ...
    const onSearchCustomerId = () => {
        let data = {
            customerId: searchCustomerId.customerId,
        }
        const headers = {
            'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
        axios.get('/customer/search-id/' + data.customerId, {headers: headers})
            .then(resp => {
                setListCustomer(resp.data.data)
            })
            .catch(error => {
    ...
    const onSearchCustomerName = () => {
        let data = {
            customerName: searchCustomerName.customerName,
        }
        const headers = {
            'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
        axios.get('/customer/search-name/' + data.customerName, {headers: headers})
            .then(resp => {
                setListCustomer(resp.data.data)
            })
            .catch(error => {
    ...
    const onSearchCustomerPhoneNumber = () => {
        let data = {
            phoneNumber: searchCustomerPhoneNumber.phoneNumber,
        }
        const headers = {
            'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
    ...
    const onSearchCustomerEmail = () => {
        let data = {
            email: searchCustomerEmail.email,
        }
        const headers = {
            'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
        axios.get('/customer/search-email/' + data.email, {headers: headers})
            .then(resp => {
                setListCustomer(resp.data.data)
            })
            .catch((error) => {
    ...
    const onSearchCustomerAddress = () => {
        let data = {
            address: searchCustomerAddress.address,
        }
        const headers = {
            'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
        axios.get('/customer/search-address/' + data.address, {headers: headers})
            .then(resp => {
                setListCustomer(resp.data.data)
            })
            .catch((error) => {
    ...
    const onSearchActive = () => {
        ...
        axios.get('/customer/search-active/' + data.activeId, {headers: headers})
            .then(resp => {
                setListCustomer(resp.data.data)
            })
            .catch(error => {
    ...
    //  Search gender ------------------------------------------------------------------------------------------------
    const [searchGenderInitial, setSearchGenderInitial] = useState({
        genderInitial: ''
    })
    const [searchGender, setSearchGender] = useState({
        genderId: searchGenderInitial.genderInitial
    })
    console.log("searchGender" + searchGender.genderId)
    const onSearchGender = () => {
        let data = {
            genderId: searchGender.genderId,
        }
        console.log("Active " + data.genderId)
        const headers = {
            'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
        axios.get('/customer/search-gender/' + data.genderId, {headers: headers})
            .then(resp => {
                setListCustomer(resp.data.data)
            })
            .catch(error => {
            ...
            <MDBContainer>
                <MDBRow>
                    <MDBCol size='md-2 mt-3'>
                        <MDBRow>
                            <MDBCol>
                                <h5 style={{color: "black"}}><b>Khách hàng</b></h5>
                            </MDBCol>
                        </MDBRow>
                    </MDBCol>
                            ...
                            <MDBCol size='md-6'>
                                <MDBDropdown className='mb-4'>
                                    <MDBDropdownToggle className='form-control text-dark bg-white text-start'>Theo mã,
                                        tên, điện thoại</MDBDropdownToggle>
                                                ...
                                                <MDBRow>
                                                    <MDBCol md='8'>
                                                        <MDBInput className={"bg-white"}
                                                                  ...
                                                                  wrapperClass='mb-4'
                                                                  label='Theo mã khách hàng' size='lg'
                                                                  type='number'/>
                                                    </MDBCol>
                                                    <MDBCol>
                                                        <MDBBtn className='btn btn-success mb-5 text-end'
                                                                onClick={onSearchCustomerId}>Tìm kiếm</MDBBtn>
                                                    </MDBCol>
                                                </MDBRow>
                                                <MDBRow>
                                                    <MDBCol md='8'>
                                                        <MDBInput className={"bg-white"}
                                                                  ...
                                                                  wrapperClass='mb-4'
                                                                  label='Theo tên khách hàng' size='lg'
                                                                  type='text'/>
                                                    </MDBCol>
                                                    <MDBCol>
                                                        <MDBBtn className='btn btn-success mb-5 text-end'
                                                                onClick={onSearchCustomerName}>Tìm kiếm</MDBBtn>
                                                    </MDBCol>
                                                </MDBRow>
                                                <MDBRow>
                                                    <MDBCol md='8'>
                                                    ...
                                                    <MDBCol>
                                                        <MDBBtn className='btn btn-success mb-5 text-end'
                                                                onClick={onSearchCustomerPhoneNumber}>Tìm kiếm</MDBBtn>
                                                    </MDBCol>
                                                </MDBRow>
                                                <MDBRow>
                                                    <MDBCol md='8'>
                                                        <MDBInput className={"bg-white"}
                                                                  ...
                                                                  wrapperClass='mb-4'
                                                                  label='Theo email' size='lg'
                                                                  type='text'/>
                                                    </MDBCol>
                                                    <MDBCol>
                                                        <MDBBtn className='btn btn-success mb-5 text-end'
                                                                onClick={onSearchCustomerEmail}>Tìm kiếm</MDBBtn>
                                                    </MDBCol>
                                                </MDBRow>
                                                <MDBRow>
                                                    <MDBCol md='8'>
                                                        <MDBInput className={"bg-white"}
                                                                  ...
                                                                  wrapperClass='mb-4'
                                                                  label='Theo address' size='lg'
                                                                  type='text'/>
                                                    </MDBCol>
                                                    <MDBCol>
                                                        <MDBBtn className='btn btn-success mb-5 text-end'
                                                                onClick={onSearchCustomerAddress}>Tìm kiếm</MDBBtn>
                                                    </MDBCol>
                                                </MDBRow>
                                            </MDBDropdownItem>
                                        </MDBContainer>
                        ...
                        <MDBRow>
                            <MDBCol>
                                <div className="w-100 p-3 shadow mb-3 bg-white rounded-4">
                                    <p><b>Giới tính</b></p>
                                    <MDBRadio name='genderId'
                                              id='genderId'
                                              value={1}
                                              onChange={inputDataSearchCustomerGender}
                                    ...
                                    <MDBRadio name='genderId'
                                              ...
                                              value={2}
                                              onChange={inputDataSearchCustomerGender}
                                              onClick={onSearchGender} label='Nam'/><br/>
                                    <MDBRadio name='genderId'
                                              id='genderId'
                                              value={3}
                                              onChange={inputDataSearchCustomerGender}
                                              onClick={onSearchGender} label='Nữ'/><br/>
                                    <MDBRadio name='genderId'
                                              id='genderId'
                                              value={4}
                                              onChange={inputDataSearchCustomerGender}
                                              onClick={onSearchGender} label='Khác'/><br/>
                                </div>
                            </MDBCol>
                        </MDBRow>
                        <MDBRow>
